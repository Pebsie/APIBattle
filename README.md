...
Set cron.php to run frequently. The default content packs are balanced for this running once a minute. **(Note that this is cycling through all world tiles and all players every time it is run. This ~_shouldn't_ be an issue, but~ is definitely an issue as you're making at least 30,000 requests to the database. See issue #6)** 

### API Reference
All interaction from here on should be with wherever `api.php` is stored on your webserver. All requests are GET requests. The following is available to you:

#### Registration
`api.php?a=login&username=*username*&password=*password` will return a `player` object or 
```{"status":"failed","reason":"A user already exists with this username!"}```
